...
#        self.levels_n[direction].pop(-2)
    ...
    def setValleyBoundary(self, z_offset, z_start, y_offset, direction, yfun=None):
        ...
        z_max = np.amax(self.channel.levels_z[direction][-1])
        z_start += z_max
        z_start = z_start - self.x_v*self.x_slope

        if direction == 'left':
            y_max = np.amax(self.channel.levels_y[direction][-1])
            y_offset += y_max
        else:
            y_max = np.amin(self.channel.levels_y[direction][-1])
    ...
    def tocsv(self, outfile):
        ...
        xyz_out = [[round(xyz[0][i]/5,3), round(xyz[1][i]/5, 3), round(xyz[2][i]/5, 3), xyz[3][i]] for i in range(len(xyz[0]))]
